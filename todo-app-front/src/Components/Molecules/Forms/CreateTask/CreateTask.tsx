...
export function CreateTaskForm({controller,state}:modalRegisterFormProps) {
  const {change,errors,submit,gradeOptions,createForm}=useCreateTaskForm()
      ...
      <Modal show={state} onHide={()=>controller(false)}>
        ...
        <Modal.Body>
            <form onSubmit={(e)=>submit(e)}>
                <StandardInput title='Nombre' change={change} name='name' error={errors.name} placeHolder='Bañar a mi mascota'/>
                <TextArea title='Descripcion' change={change} name='description' error={errors.description} placeHolder='Descripcion de la tarea creada' />
                <NumberInput title='Tiempo (minutos)' change={change} name='limitTime' error={errors.limitTime} placeHolder='0' />
                <SelectInput options={gradeOptions} change={change} value={createForm.grade} name='grade' title='Grado de la tarea'/>
            </form>
        </Modal.Body>
        <Modal.Footer>
