...
export const ContainerDD = ({
  ...
  const handleDrop = (e: React.DragEvent<HTMLDivElement>) => {
    ...
    editTask({
      ...
      props: {
        status,
        finishDate: status === "Success" ? new Date().toISOString() : null,
      },
    });
    handleDraggin(false);
  };
  ...
  return (
    <div
      className={`layoutCards${isDraggin ? " layoutDraggin" : ""}`}
    ...
    >
      ...
      <hr />
      {data
        .filter((el) => filt(el.limitTime, filter))
        .map((el, ind) => (
